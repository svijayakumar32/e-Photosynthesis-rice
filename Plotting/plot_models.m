...
for j = 1:length(CA) %orig 25->50
    ...
    Farq_Matrix(k, 1) = Vrubusco_nonopt; % just a sanity check / to align the columns with ePhoto
    Farq_Matrix(k, 2) = VmaxAdj_nonopt; % just a sanity check / to align the columns with ePhoto
...
for j = 1:length(CA)
    ...
    Eio(1) = Edata.data(1,1) * Vrubusco_nonopt; % select alpha vals here
    Eio(2:27) = Edata.data(2:27,1) * VmaxAdj_nonopt;
    ...
    % Fill in results matrix
        ePhoto_Matrix(k, 1) = Vrubusco_nonopt; % should be same values all the way down
        ePhoto_Matrix(k, 2) = VmaxAdj_nonopt; % should be same values all the way down
