...
R=8.314;
% 18/12/2023
% Should they be modified to match tobacco?
% Supposedly,the original numbers are from C.Bernacchi's thesis (Table 3.1)  
% c_c=38.28;   % Kc c?
% dHa_c=80.99; % Kc dHa?
% c_o=14.68;   % Ko c?
% dHa_o=23.72; % Ko dHa?
% dHa values are same as Sharkeys tobacco but c values are not 
% Kinetic constants for tobacco
c_c = 35.9774;   % Kc c
dHa_c = 80.99;   % Kc dHa
c_o = 12.3772;   % Ko c
dHa_o = 23.72;   % Ko dHa
...
%18/12/23 - Are there different PsKM11/PsKM12 values for tobacco?
% Using Zhu et al 2013 values (0.0115,0.222) instead of soy (0.0097,0.244)
PsKM11_0	=	0.0115;     % 	CO2	1	RuBP+CO2->2PGA
PsKM12_0	=	0.222;		%	O2	1	RuBP+CO2->2PGA  0.28 DEFAULT. 

Kc_at_25 = exp(c_c-dHa_c*1000/(R*(25+273.15)))*10; 
Ko_at_25 = exp(c_o-dHa_o*1000/(R*(25+273.15)))*10;

Kc_at_Tp = exp(c_c-dHa_c*1000/(R*(25+273.15)))*10; 
Ko_at_Tp = exp(c_o-dHa_o*1000/(R*(25+273.15)))*10;

%Original
%PsKM11=PsKM11_0*exp(c_c-dHa_c*1000/(R*(Tp+273.15)))/272.38; The denominator is a scaling factor of Kc at 25 C
%PsKM12=PsKM12_0*exp(c_o-dHa_o*1000/(R*(Tp+273.15)))/165.82; The denominator is a scaling factor of Ko at 25 C

%New
PsKM11=PsKM11_0*exp(c_c-dHa_c*1000/(R*(Tp+273.15)))/(Kc_at_25); %Kc=272.37, Tobacco at 25C
PsKM12=PsKM12_0*exp(c_o-dHa_o*1000/(R*(Tp+273.15)))/(Ko_at_25); %Ko=165.79, Tobacco at 25C
...
% First here is one way of the redox regulation, assuming the regulation is instantaneous.
