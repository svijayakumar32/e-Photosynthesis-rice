...
% ORIGINALLY WRONG WAY AROUND - AFFECTING RESULT?
% global Vrubusco_opt;
...
Vrubusco_opt=0.96;%adjust enzyme activity sub in optimal Vrubisco from Vcmax_adj_simple_3 
VmaxAdj_opt=0.98;%adjust enzyme activity i.e. sub in optimal Vmaxadj from Jmax_adj_simple_3
...
%% using initial (a_Enzymes/Rubisco=1) and adjusted (a_Enzymes=0.98, a_Rubisco=0.96) scaling factors
...
% Adjust enzyme activities for e_Photosynthesis - nonopt is both alpha values = 1, opt is aRubisco = 0.96, aEnzymes = 0.98 
    Eio_nonopt(1) = Edata.data(1,1) * Vrubusco_nonopt;      %aRubisco = 1
    Eio_nonopt(2:27) = Edata.data(2:27,1) * VmaxAdj_nonopt; %aEnzymes = 1
    Eio_opt(1) = Edata.data(1,1) * Vrubusco_opt;            %aRubisco = 0.96
    Eio_opt(2:27) = Edata.data(2:27,1) * VmaxAdj_opt;       %aEnzymes = 0.98  
...
%  Calculate Gross Assimilation for e-Photosynthesis model
    ...
    Gross_A_opt(i) = EPS_Drive_GRNs(Einput,Cc_ePhoto(i),PPFDi,WeatherTemp,GRNC,0,Eio_opt); %aRubisco = 0.96, aEnzymes = 0.98
end

save('Farq_ePhoto_results.mat');
%%%%%%%%%%%%%%%%%%%%%