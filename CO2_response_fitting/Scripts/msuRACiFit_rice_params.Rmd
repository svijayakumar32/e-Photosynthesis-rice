...
```{r}
base_path <- here("Outputs", "Parameters")
```
...
```{r}
Gstars_rice_file <- file.path(base_path, "Gstars_rice.csv")
write.csv(Gstars_rice,Gstars_rice_file) #ppm
```

```{r}
Gstars_Pa_rice_file <- file.path(base_path, "Gstars_Pa_rice.csv")
...
all_param_file <- file.path(base_path, "kinetic_parameters_rice.csv")
...
for (o in seq_along(complete_Fits_rice_list)) {
  ...
  output_filename <- file.path(base_path,filename)
...
## Produce data tables using parameters fitted for each curve and the SSR assoaciated with each fit:
...
```{r}
SSR_file <- file.path(base_path, "SSR_values_rice.csv")
...
```{r}
limitations_file <- file.path(base_path, "completeFit_rice_limitations.csv")
